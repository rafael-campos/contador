...
const TimelinePath: React.FC<{ scrollProgress: MotionValue<number> }> = ({ scrollProgress }) => {
  ...
  // Adjust the path length based on the number of items or desired height
  ...
  return (
    <motion.svg
      width="20"
      height="100%"
      viewBox="0 0 20 800" // Adjust viewBox height if pathDefinition changes
    ...
    >
      <motion.path
        ref={pathRef}
        d={pathDefinition}
        stroke="url(#path-gradient-memory)" // Use a unique ID for the gradient
        strokeWidth="3"
        fill="none"
        style={{ pathLength: scrollProgress }}
      />
      <defs>
        <linearGradient id="path-gradient-memory" x1="0%" y1="0%" x2="0%" y2="100%">
...
const MemoryTimeline: React.FC<{ memories: Memory[] }> = ({ memories }) => {
